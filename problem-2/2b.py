...
def do_inverse_log(image):
    ...
    # transformed_image = np.uint8(scalling_factor * np.log(1 + transformed_image))

    # for inverse log operation
    f_value = (np.exp(f_range) ** (1/scalling_factor)) - 1
    transformed_image = np.uint8(np.exp(transformed_image*255) ** (1/scalling_factor) - 1)
    return transformed_image, f_value, f_range
